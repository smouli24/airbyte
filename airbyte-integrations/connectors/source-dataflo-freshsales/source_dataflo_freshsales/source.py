...
class DatafloFreshsalesStream(HttpStream, ABC):  
...
class Contacts(DatafloFreshsalesStream):
    """
    ...
    def request_params(
        ...
        params = {"page": self.page, "sort": self.order_field, "sort_type": "asc","include":"lifecycle_stage"}
        return params


class Accounts(DatafloFreshsalesStream):
    """
...
class Deals(DatafloFreshsalesStream):
    object_name = "deals"
...
class OpenTasks(DatafloFreshsalesStream):
    """
...
class CompletedTasks(DatafloFreshsalesStream):
    """
    API docs: https://developers.freshworks.com/crm/api/#tasks
    """
...
class PastAppointments(DatafloFreshsalesStream):
    """
...
class UpcomingAppointments(DatafloFreshsalesStream):
    """
    API docs: https://developers.freshworks.com/crm/api/#appointments
    """
...
class LifecycleStages(DatafloFreshsalesStream):
    """
    API docs: https://developers.freshworks.com/crm/api/#contacts
    """

    object_name = "selector/lifecycle_stages"
    

    def path(
        self, stream_state: Mapping[str, Any] = None, stream_slice: Mapping[str, Any] = None, next_page_token: Mapping[str, Any] = None
    ) -> str:
        self.logger.info(f"The following scopes were granted: ABCDEFGHIJKLMNOP++======================")
        return f"{self.object_name}"
    
    def parse_response(self, response: requests.Response, **kwargs) -> Iterable[Mapping]:
        json_response = response.json()
        self.logger.info(f"JSON Response ======================{json_response}")
        records = json_response.get("lifecycle_stages", []) if self.object_name is not None else json_response
        self.logger.info(f"Records ======================{records}")
        yield from records

# Source
class SourceDatafloFreshsales(AbstractSource):
